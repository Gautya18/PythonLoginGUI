...
class Member():
    ...
    def find(self, un,pw):
        #----- Database Connection ----------#
        conn = cx_Oracle.connect('2017BCGRP12/2017BCGRP12@192.168.43.216/pdborcl')
        curs = conn.cursor();

        curs.execute("SELECT * FROM member WHERE username = :name AND passwd = :ps", {"name":un,"ps":pw})
...
def Save(exist):
    ...
    curs = conn.cursor();
    if (exist == True):
        curs.execute("UPDATE member SET passwd = :a , first_nm = :b , last_nm = :c , phone = :d , email = :e WHERE username = :f",{"a":psw.get(),"b":fnm.get(),"c":lnm.get(),"d":phn.get(),"e":eml.get(),"f":unm.get()})
    else:
        curs.execute("INSERT INTO member (username, passwd, first_nm, last_nm, phone, email) VALUES (:a,:b,:c,:d,:e,:f)",{"a":unm.get(),"b":psw.get(),"c":fnm.get(),"d":lnm.get(),"e":phn.get(),"f":eml.get()})
    conn.commit()
    curs.close()
    conn.close()
    Back(Home)        
